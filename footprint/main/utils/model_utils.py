...
def model_fields(manager, fields):
    ...
    return filter(
        lambda field: not fields or field.name in fields, manager.model._meta.fields
    )


def field_predicate(field, exclude_field_types, fields, excludes):
    return (
        (not fields or field.name in fields)
        and not isinstance(field, exclude_field_types)
        and not field.name in excludes
    )
...
def index_of_field_for_sort(field_names, for_values_queryset):
    ...
    def sort_priority(field):
        ...
        return (
            (field_names or []).index(name)
            if name in (field_names or [])
            else sys.maxint
        )
...
def model_field_paths(
    ...
    return compact(
        map(
            lambda field: (
                (field.attname if for_value_queryset else field.name)
...
def _field_path_and_cloned_field(related_field, field, separator):
    field_path = string.replace(
        "{0}__{1}".format(related_field, field.name), "__", separator or "__"
    )
...
def resolve_db_entity_key_of_field_path(
    field_path_or_f_statement, manager, related_models
):
    ...
    try:
        ...
        path = (
            field_path_or_f_statement.name
            if isinstance(field_path_or_f_statement, F)
            else field_path_or_f_statement
        )
        # If an absolute path, take the first segment and try to resolve it as a db_entity_key
        first_segment = path.split("__")
        try:
            if (
                manager.model.config_entity.computed_db_entities(
                    key=first_segment
                ).count()
                > 0
            ):
            ...
            return manager.model.config_entity.db_entity_key_of_feature_class(
                manager.model
            )
    except:
        raise Exception(
    ...
    while len(parts) > 0:
        ...
        if not model_or_field_tuple:
            ...
            related_field_tuple = get_value_of_first_matching_key(
                lambda name, field: (
                    part == field[0].attname if hasattr(field[0], "attname") else False
                ),
                model._meta._name_map,
            )
            if related_field_tuple:
                model_or_field_tuple = related_field_tuple[0].rel.to._meta._name_map[
                    "id"
                ]
        ...
        elif hasattr(model_or_field_tuple[0], "model"):
        ...
        return (
            model_or_field_tuple[0]
            if not return_related_models
            else (model_or_field_tuple[0], related_model or None)
        )
...
def resolve_field(model_class, field_name):
    ...
    """
    try:
        return filter(lambda field: field.name == field_name, model_class._meta.fields)[
            0
        ]
    except Exception as e:
        raise Exception(
            "Model class {model_class} has no field {field_name}".format(
                model_class=model_class, field_name=field_name
            )
        )
...
def resolve_field_of_type(model, target_class):
    ...
    """
    return filter(
        lambda field: field.rel and field.rel.to and field.rel.to == target_class,
        model._meta.fields,
    )[0]
...
def resolve_queryable_name_of_type(model, target_class):
    ...
    m2m_results = filter(
        lambda related_object: related_object[0].model == target_class,
        model._meta.get_all_related_m2m_objects_with_model(),
    )
    if len(m2m_results) == 1:
        return m2m_results[0][0].var_name
    raise Exception(
        "No queryable name matching target_class %s for model %s"
        % (target_class, model)
    )
...
def limited_api_fields(model, for_template=False):
    ...
    """
    return (
        model.limited_api_fields(for_template)
        if hasattr(model, "limited_api_fields") and model.limited_api_fields()
        else None
    )
...
def form_module_name(module, module_fragment, schema):
    return (
        "%s.%s.%s_%s" % (schema, module, schema, module_fragment)
        if module
        else "%s.%s_%s" % (schema, schema, module_fragment)
    )
...
def model_dict(
    ...
    return map_to_dict(
        lambda field: field_value(model_instance, field, include_null),
        filter(
            lambda field: not (field.primary_key and not include_primary_key)
            and field.name not in omit_fields,
