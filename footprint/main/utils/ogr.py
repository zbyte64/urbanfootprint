...
class OGRETL(object):
    ...
    EXPORT_COMMAND = (
        "/usr/local/bin/ogr2ogr -append -f {{output_format}} {{geom_type}} {{export_file}} "
        'PG:"{db_connection}" {{source}} {{options}}'.format(
            db_connection=DB_CONNECTION
        )
    )

    IMPORT_COMMAND = (
        '/usr/local/bin/ogr2ogr -overwrite -skipfailures -progress -f "PostgreSQL"'
        'PG:"{db_connection}" {{source}} {{options}}'.format(
            db_connection=DB_CONNECTION
        )
    )
    ...
    def __init__(self):
        self.import_drivers = {
            driver: options
            for driver, options in fiona.supported_drivers.items()
            if "r" in options
        }
        self.export_drivers = {
            driver: options
    ...
    def test(self):
        layers = {
            "transit_stops": "select * from scag__or_scenarios.major_transit_stops",
        ...
        for output, layer in tests:
            ...
            assert not result.returncode, "ERROR: {layer} > {output}: {err}".format(
                layer=layer[0], output=output, err=result.stderr.text
            )
    ...
    def export(self, name, statement, output_format, geom="wkb_geometry"):
        ...
        output_file = "{dir}/{name}.{ext}".format(
            name=name,
        ...
        formatted_options = [
            option.format(**options_formatting)
            for option in self.FORMATS[output_format].get("options", [])
        ]
        ...
        assert not result.returncode, "ERROR: {name} > {output}: {err}".format(
            name=name, output=output_format, err=result.stderr.text
        )
