...
class Feature(models.Model, Revisionable, Dynamic):
    ...
    dynamic_model_class_creator_class = (
        "footprint.main.models.feature.feature_class_creator.FeatureClassCreator"
    )
    ...
    def the_unique_id(self):
        ...
        return "%s_%s" % (
            (
                self.db_entity.id
                if self.configuration.key != FeatureClassCreator.DETACHED_FROM_DB_ENTITY
                else FeatureClassCreator.DETACHED_FROM_DB_ENTITY
            ),
            self.id,
        )
    ...
    def limited_api_fields(cls, for_template=False):
        ...
        return (
            (
                ["id"]
                + cls.api_include
                + (cls.template_api_fields if for_template else [])
                + cls.default_api_fields
            )
            if cls.api_include
            else None
        )
    ...
    def result_map(cls):
        ...
        if not feature_class_creator.dynamic_model_class_is_ready:
            return None
        cls._result_map = (
            feature_class_creator.dynamic_model_class()
            .objects.all()
            .create_result_map()
        )
        return cls._result_map
...
class PaintingFeature(Feature, BuiltFormFeature):
    ...
    developable_proportion = models.DecimalField(
...
class FeatureGeography(models.Model):
    ...
    def __unicode__(self):
        return "FeatureGeography:Feature:{1}, Geography:{2}".format(
            self.feature, self.geography
        )
