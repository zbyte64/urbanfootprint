...
class Behavior(Name, Key, Tags, Deletable):
    ...
    def __init__(self, *args, **kwargs):
        super(Behavior, self).__init__(
            *args,
            **remove_keys(kwargs, ["parents", "tags", "template_feature_behavior"])
        )
    ...
    def update_or_create_associations(self, behavior):
        ...
        self.template_feature_behavior = (
            self.update_or_create_template_feature_behavior(
                behavior._template_feature_behavior
            )
        )
        ...
        if behavior._parents:
            ...
            self.parents.add(
                *Behavior.objects.filter(
                    key__in=map(
                        lambda parent: BehaviorKey.Fab.ricate(parent), behavior._parents
                    )
                )
            )
        if behavior._tags:
        ...
        intersection = behavior._intersection or (
            self.parents.all()[0].intersection_subclassed
            if self.parents.count() == 1
            else GeographicIntersection.polygon_to_polygon
        )
        if not intersection:
        ...
        self.intersection = intersection.__class__.objects.get_or_create(
            **model_dict(intersection)
        )[0]
        self.save()
    ...
    intersection = models.ForeignKey(Intersection, null=True, on_delete=models.PROTECT)
    ...
    def intersection_subclassed(self):
        ...
        """
        return (
            Intersection.objects.get_subclass(id=self.intersection.id)
            if (self.intersection and self.intersection.id)
    ...
    template_feature_behavior = models.ForeignKey(
        "FeatureBehavior",
        null=True,
        related_name="owning_behavior",
    ...
    def subclassed_template_feature_behavior(self):
        ...
        if not id in Behavior._subclassed_template_feature_behavior:
            ...
            Behavior._subclassed_template_feature_behavior[id] = (
    ...
    def update_or_create_template_feature_behavior(self, template_feature_behavior):
        ...
        return (
            template_feature_behavior.__class__
            if template_feature_behavior
            else FeatureBehavior
        ).objects.update_or_create(
            behavior=self,
            db_entity=DbEntity.objects.get(
                key="template_feature_behavior"
            ),  # Special DbEntity
            is_template=True,
            defaults=(
    ...
    def feature_behavior_from_behavior_template(self):
        ...
        intersection = (
            template_feature_behavior.intersection_subclassed
            or self.intersection_subclassed
        )
        if not intersection:
        ...
        logger.debug(
            ...
            % (
                (
                    model_dict(intersection, include_primary_key=True)
                    if intersection
                    else "None"
                ),
                intersection.__class__ if intersection else "None",
            )
        )
    ...
    def _computed_tags(self, seen=[]):
        ...
        return accumulate(
            ...
            map(
                lambda parent: parent._computed_tags(seen + list(self.parents.all())),
                filter(
                    lambda parent: parent not in seen, self.parents.all().distinct()
                ),
            ),
        ).distinct()
    ...
    def computed_behaviors(self, level=0):
        ...
        return [self] + flat_map(
            lambda parent: parent.computed_behaviors(), self.parents.all()
        )
    ...
    def has_behavior(self, behavior, seen=[]):
        return self == behavior or any_true(
            lambda parent_behavior: parent_behavior.has_behavior(
                behavior, seen + [self]
            ),
            filter(lambda behavior: behavior not in seen, self.parents.all()),
        )
    ...
    def __unicode__(self):
        return "{0} (Parents: {1})".format(
            self.key,
            ", ".join(
                map(
                    lambda parent: parent.key,
                    self.parents.all() if self.pk else self._parents,
                )
            ),
        )
