...
from footprint.main.models.geospatial.attribute_group_configuration import (
...
class FeatureBehavior(Tags):
    ...
    behavior = models.ForeignKey(Behavior, null=False, on_delete=models.PROTECT)
    ...
    attribute_groups = ManyToManyField(
        AttributeGroup, through=AttributeGroupConfiguration
    )

    # The DbEntity of the FeatureBehavior. Only null temporarily when being hydrate by Tastypie during a DbEntity save
    db_entity = models.ForeignKey(DbEntity, null=True, on_delete=models.PROTECT)
    ...
    def __init__(self, *args, **kwargs):
        super(FeatureBehavior, self).__init__(
            *args, **remove_keys(kwargs, ["tags", "attribute_group_configurations"])
        )
        self._tags = self._tags or []
        self._tags.extend(kwargs.get("tags", []))
        self._attribute_group_configurations = (
            kwargs.get("attribute_group_configurations")
            or self._attribute_group_configurations
            or []
        )
    ...
    def set_defaults(self):
        ...
        self.intersection = (
            self.intersection_subclassed or self.behavior.intersection_subclassed
        )
    ...
    def update_or_create_associations(self, feature_behavior):
        ...
        if feature_behavior._attribute_group_configurations:
            ...
            for (
                attribute_group_configuration
            ) in feature_behavior._attribute_group_configurations:
                attribute_group_configuration, created, updated = (
                    AttributeGroupConfiguration.objects.update_or_create(
