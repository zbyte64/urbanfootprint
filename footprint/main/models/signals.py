...
from footprint.main.models.signal_handlers import (
    through_item_added,
    through_item_deleted,
    items_changed,
)
...
for attribute in ConfigEntity.INHERITABLE_COLLECTIONS:
    ...
    if has_explicit_through_class(ConfigEntity, attribute):
        ...
        post_save.connect(
            through_item_added(attribute), sender=through_class, weak=False
        )
        pre_delete.connect(
            through_item_deleted(attribute), sender=through_class, weak=False
        )
    else:
...
m2m_changed.connect(
    items_changed("layers"), sender=layer_library_through_class, weak=False
)
result_library_through_class = ResultLibrary.results.through
m2m_changed.connect(
    items_changed("results"), sender=result_library_through_class, weak=False
)
