...
def items_changed(attribute):
    def _items_changed(sender, **kwargs):
        ...
        logger.debug(
            "Item changed for donor %s. Applying changes to donees %s",
            donor,
            ", ".join(donee.name for donee in donees),
        )
        if action == "post_add":
            ...
            for donee in donees:
                ...
                if len(manager.all()) > 0:
                    added = getattr(donor, attribute).filter(
                        pk__in=kwargs["pk_set"], deleted=False
                    )
