...
from footprint.main.models.config.model_pickled_object_field import (
    ModelPickledObjectField,
)
...
class PolicyLookup(object):
    ...
    def annualize_benchmark_years(self, base_year, future_year):
        ...
        for year in benchmark_years:
            ...
            yearly_efficiency_dict.update(
                {
                    y: (1 - previous_year_efficiency)
                    - ((abs(previous_year - y)) * annual_efficiency_improvement)
    ...
    def get_building_efficiency_assumptions(
        self, types, uses=["commercial", "residential"]
    ):
        """
        ...
        for type, use in product(types, uses):
            ...
            assumption_dict = {
                ...
                "{use}_{type}_new_construction_efficiency".format(
                    **assumption
                ): get_policy(
                    "{use}_{type}_new_construction_efficiency".format(**assumption)
                ),
                "{use}_{type}_new_retrofit_efficiency".format(**assumption): get_policy(
                    "{use}_{type}_new_retrofit_efficiency".format(**assumption)
                ),
                "{use}_{type}_base_retrofit_efficiency".format(
                    **assumption
                ): get_policy(
                    "{use}_{type}_base_retrofit_efficiency".format(**assumption)
                ),
                # rates for building turnover
                "{use}_{type}_annual_new_retrofit_rate".format(**assumption): float(
                    get_policy(
                        "{use}_{type}_rates.annual_new_retrofit_rate".format(
                            **assumption
                        )
                    )
                ),
                "{use}_{type}_annual_base_retrofit_rate".format(**assumption): float(
                    get_policy(
                        "{use}_{type}_rates.annual_base_retrofit_rate".format(
                            **assumption
                        )
                    )
                ),
                "{use}_{type}_annual_base_replacement_rate".format(**assumption): float(
                    get_policy(
                        "{use}_{type}_rates.annual_base_renovation_rate".format(
                            **assumption
                        )
                    )
                ),
...
class Policy(SharedKey, Name, Tags, PolicyLookup):
    ...
    def update_or_create_policy(self, policy_config):
        child_policy = Policy.objects.update_or_create(
            ...
            schema=(
                "%s__%s" % (self.schema, policy_config["key"])
                if self.schema
                else policy_config["key"]
            ),
            defaults=dict(
        ...
        if policy_config.get("policies", None) and len(policy_config["policies"]) > 0:
            child_policy.policies.add(
                *map(
                    lambda child_policy_config: child_policy.update_or_create_policy(
                        child_policy_config
                    ),
