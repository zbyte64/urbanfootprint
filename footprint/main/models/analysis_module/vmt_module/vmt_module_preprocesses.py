...
from footprint.main.models.analysis_module.geoprocessing_functions import (
...
def add_analysis_geom(schema, table):
    ...
    pSql = """alter table {schema}.{table} add column analysis_geom geometry;""".format(
        schema=schema, table=table
    )
    try:
        execute_sql(pSql)
...
def run_vmt_one_mile_buffers(sql_config_dict):
    ...
    add_analysis_geom(
        sql_config_dict["vmt_variables_schema"], sql_config_dict["vmt_variables_table"]
    )
    add_analysis_geom(
    ...
    aggregate_within_distance(
        dict(
            source_table=sql_config_dict["uf_canvas_schema"]
            + "."
            ...
            variable_field_dict=dict(emp_1mile=["emp"]),
        )
    )
...
def run_vmt_quarter_mile_buffers(sql_config_dict):
    ...
    Alter Table {schema}.{table} drop column analysis_geom;""".format(
        schema=sql_config_dict["transit_stop_schema"],
...
def run_vmt_variable_trip_length_buffers(sql_config_dict):
    ...
    drop_table(
        "{vmt_variables_schema}.{vmt_variables_table}_vmt_variable".format(
            vmt_variables_schema=sql_config_dict["vmt_variables_schema"],
            vmt_variables_table=sql_config_dict["vmt_variables_table"],
        )
    )
    ...
    add_geom_idx(
        sql_config_dict["vmt_variables_schema"],
        sql_config_dict["vmt_variables_table"] + "_vmt_variable",
    )
    add_primary_key(
        sql_config_dict["vmt_variables_schema"],
        sql_config_dict["vmt_variables_table"] + "_vmt_variable",
    ...
    aggregate_within_variable_distance(
        dict(
            source_table=sql_config_dict["vmt_variables_schema"]
            + "."
            + sql_config_dict["vmt_variables_table"]
            + "_vmt_variable",
    ...
    pSql = """DROP INDEX {schema}.{schema}_{table}_analysis_geom;
    Alter Table {schema}.{table} drop column analysis_geom;""".format(
        schema=sql_config_dict["vmt_variables_schema"],
