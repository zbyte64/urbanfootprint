...
class AgricultureUpdaterTool(AnalysisTool):
    ...
    def update_dependent_scenarios(self, base_features, scenario):
        if isinstance(scenario, BaseScenario):
            ...
            logger.info(
                "Updating dependent scenarios {0} of {1}".format(
            ...
            for future_scenario in future_scenarios:
                ...
                logger.info(
                    "Updating {0} features of {1}".format(
                ...
                layer = Layer.objects.filter(
                    presentation__config_entity=agriculture_feature_class.config_entity,
                    db_entity_interest__db_entity__key=agriculture_feature_class.db_entity_key,
                )[0]
    ...
    def update(self, **kwargs):
        ...
        logger.debug(
            "{0}:Starting Agriculture Core Analysis for {1}".format(
        ...
        if feature_count <= 36:
            increment_portion = (0.9 / feature_count) + 0.001
            equal_portion = 1
        else:
            increment_portion = 0.05
        ...
        for feature in features.iterator():
            ...
            if not feature.built_form:
                ...
                applied_acres = (
                    feature.acres_gross * feature.density_pct * feature.dev_pct
                )
                agriculture_attribute_set = feature.built_form.resolve_built_form(
                    feature.built_form
                ).agriculture_attribute_set
                feature.built_form_key = feature.built_form.key
                feature.crop_yield = (
                    agriculture_attribute_set.crop_yield * applied_acres
                )
                feature.market_value = (
                    agriculture_attribute_set.unit_price * feature.crop_yield
                )
                feature.production_cost = agriculture_attribute_set.cost * applied_acres
                feature.water_consumption = (
                    agriculture_attribute_set.water_consumption * applied_acres
                )
                feature.labor_force = (
                    agriculture_attribute_set.labor_input * applied_acres
                )
                feature.truck_trips = (
                    agriculture_attribute_set.truck_trips * applied_acres
                )
            feature.save(update_fields=self.ANALYSIS_FIELDS)
            i += 1
        ...
        logger.debug(
            "Processed {0} features in {1}: {2} per feature".format(
                feature_count, total_time, total_time / feature_count
            )
        )
        self.progress(0.9, **kwargs)
        logger.debug(
            "{0}:Finished Agriculture Core Analysis for {1} ".format(
                timestamp(), self.config_entity
            )
        )
