...
def calculate_distance(distance_options):
    ...
    zero_values_check = report_sql_values(
        """select
              sum(*)
              from {source_table} where {source_table_query};""".format(
            **distance_options
        ),
        "fetchone",
    )
    ...
    pSql = """drop function if exists distance_tool(
      ...
      out id int,
      out {column} float) cascade;""".format(
        **distance_options
    )
    ...
    drop_table(
        "{target_table_schema}.{target_table}_{column}".format(**distance_options)
    )
    ...
    """.format(
        bottom_range_id="{start_id}", top_range_id="{end_id}", **distance_options
    )
    ...
    for i in range(thread_count):
        ...
        rows_to_process = {"start_id": id_list[offset], "end_id": id_list[last_thread]}
    ...
    add_attribute_idx(
        distance_options["target_table_schema"],
        "{target_table}_{column}".format(**distance_options),
        "id",
    )
...
def aggregate_within_distance(distance_options):
    ...
    output_field_format = create_sql_calculations(
        source_table_column_list, sql_format, ", "
    )

    sql_format = (
        "cast({aggregation_type}({formatter}) as float) as {formatter}_{suffix}".format(
            formatter="{0}", **distance_options
        )
    )
    sql_calculations_format = create_sql_calculations(
        source_table_column_list, sql_format, ", "
    )
    ...
    drop_table(
        "{target_table_schema}.{target_table}_{suffix}".format(
    ...
    sql_format = "{formatter}_{suffix} float".format(
        formatter="{0}", **distance_options
    )
    output_table_field_format = create_sql_calculations(
        source_table_column_list, sql_format, ", "
    )
    ...
    pSql = """create table {target_table_schema}.{target_table}_{suffix} (id int, {output_table_field_format});""".format(
        ...
        target_table=distance_options["target_table"],
        suffix=distance_options["suffix"],
        output_table_field_format=output_table_field_format,
    )
    ...
    if len(distance_options["variable_field_dict"]) > 0:
        for key, value in distance_options["variable_field_dict"].items():
            update_table_field_format = create_sql_calculations(
                value,
                "{formatter}_{suffix}".format(formatter="b.{0}", **distance_options),
                " + ",
            )
            if count == 1:
                update_sql_format += (
...
def aggregate_within_variable_distance(distance_options):
    ...
    sql_format = "out {formatter} float".format(formatter="{0}")
    output_field_format = create_sql_calculations(
        distance_options["variable_field_list"], sql_format, ", "
    )

    sql_format = "cast({aggregation_type}({formatter}) as float) as {formatter}".format(
        formatter="{0}", aggregation_type=distance_options["aggregation_type"]
    )
    sql_calculations_format = create_sql_calculations(
        distance_options["variable_field_list"], sql_format, ", "
    )
    ...
    sql_format = "{formatter} float".format(
        formatter="{0}", suffix=distance_options["suffix"]
    )
    output_table_field_format = create_sql_calculations(
        distance_options["variable_field_list"], sql_format, ", "
    )
    ...
    update_table_field_format = create_sql_calculations(
        distance_options["variable_field_list"],
        "{0} = (case when b.{0}_var is null then 0 else b.{0}_var end)",
        ", ",
    )
    select_format = create_sql_calculations(
        distance_options["variable_field_list"], "{0} as {0}_var", ", "
    )
