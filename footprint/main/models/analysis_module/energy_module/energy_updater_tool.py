...
class EnergyUpdaterTool(AnalysisTool, BuildingPerformance):
    ...
    def use_breakout_fields(self):
        return [
            "{use}_{type}_use".format(use=use, type=e_type)
            for use, e_type in product(self.all_types_and_categories, self.METRICS)
        ]
    ...
    def run_analysis(self, **kwargs):
        ...
        self.energy_class = self.config_entity.db_entity_feature_class(
            DbEntityKey.ENERGY
        )
        ...
        if isinstance(self.config_entity.subclassed, FutureScenario):
            ...
            energy_output_list, options = self.run_future_calculations(
                options, **kwargs
            )
        else:
        ...
        logger.info(
            "Writing to feature table {energy_schema}.{energy_result_table}".format(
                **options
            )
        )
        ...
        logger.info(
            "Writing to relative table {energy_schema}.{rel_table}".format(
                energy_schema=options["energy_schema"], rel_table=self.rel_table
            )
        )
        ...
        create_and_populate_relations(
            self.config_entity,
            self.config_entity.computed_db_entities(key=DbEntityKey.ENERGY)[0],
        )
    ...
    def collect_climate_rates(self, feature):
        ...
        title_24_fields = [
            "_".join(p) for p in product(self.RESIDENTIAL_TYPES, self.METRICS)
        ]
        title_24_rates = {
            field: float(getattr(t24z, field)) for field in title_24_fields
        }

        forcasting_zone_fields = [
            "_".join(p) for p in product(self.COMMERCIAL_TYPES, self.METRICS)
        ]
        forcasting_zone_rates = {
            field: float(getattr(fcz, field)) for field in forcasting_zone_fields
        }
    ...
    def get_feature_input(self, feature):
        return dict(
            ...
            hh=feature.hh,
            emp=feature.emp,
            total_commercial_sqft=sum(
                [
                    float(getattr(feature, "bldg_sqft_" + category))
    ...
    def run_future_calculations(self, options, **kwargs):
        ...
        self.policy_set = self.config_entity.computed_policy_sets()[0].policy_by_key(
            "energy"
        )
        self.policy_assumptions = self.policy_set.get_building_efficiency_assumptions(
            self.METRICS
        )
        ...
        for feature in annotated_features.iterator():
            ...
            self.future_occupancy_rate = float(
                self.feature.hh / self.feature.du if self.feature.du else 0
            )
            self.base_occupancy_rate = float(
                self.base_canvas.hh / self.base_canvas.du if self.base_canvas.du else 0
            )
                ...
                self.feature_dict.update(
                    {
                        ...
                        + "_redev": self.redev_units(
                            "bldg_sqft_" + category, self.feature, self.base_canvas
                        ),
                        category
                        + "_new": self.new_units(
                            "bldg_sqft_" + category, self.feature, self.base_canvas
                        ...
                        category
                        + "_base": float(
                            getattr(self.base_canvas, "bldg_sqft_" + category)
                        ),
                    }
                )
                ...
                self.feature_dict.update(
                    {
                        category
                        + "_redev": self.redev_units(
                            category, self.feature, self.base_canvas
                        )
                        * self.future_occupancy_rate,
                        category
                        + "_new": self.new_units(
                            category, self.feature, self.base_canvas
                        )
                        * self.future_occupancy_rate,
                        category
                        + "_base": float(getattr(self.base_canvas, category))
                        * self.base_occupancy_rate,
                    }
                )
    ...
    def calculate_future_energy(self):
            ...
            self.result_dict.update(
                {
                    ...
                    "{use}_{metric}_use".format(**fmt): new_use
                    + base_use
                    - reduced_use,
                }
            )
        ...
        for metric in self.METRICS:
            commercial_use = sum(
                [
                    self.result_dict[
                        "{use}_{metric}_use".format(use=use, metric=metric)
                    ]
            ...
            self.result_dict.update(
                {
                    ...
                    "total_{metric}_use".format(metric=metric): commercial_use
                    + residential_use,
                }
            )
    ...
    def calculate_visualized_field(self):
        ...
        if total_units:
            self.result_dict["annual_million_btus_per_unit"] = (
                electric_btus + gas_btus
            ) / (total_units * 1000000)
        else:
    ...
    def calculate_base_use(self):
        ...
        for energy_type in self.METRICS:
            ...
            for use in types:
                ...
                self.result_dict[
                    "{0}_{1}_use".format(use, energy_type)
                ] += subuse_consumption

                if use in self.RESIDENTIAL_TYPES:
                    self.result_dict[
                        "residential_{0}_use".format(energy_type)
                    ] += subuse_consumption

                elif use in self.COMMERCIAL_TYPES:
                    self.result_dict[
                        "commercial_{0}_use".format(energy_type)
                    ] += subuse_consumption
    ...
    def write_results_to_database(self, options, energy_output_list):
        ...
        attribute_list = filter(
            lambda x: x not in ["id", "title24_zone", "fcz_zone"], self.output_fields
        )

        output_field_syntax = (
            "id int, title24_zone int, fcz_zone int, "
            + create_sql_calculations(attribute_list, "{0} numeric(14, 4)")
        )
        ...
        copy_from_text_to_db(
            output_textfile, "{energy_schema}.{energy_result_table}".format(**options)
        )
        ...
        add_geom_idx(
            options["energy_schema"], options["energy_result_table"], "wkb_geometry"
        )
        add_primary_key(options["energy_schema"], options["energy_result_table"], "id")
        add_attribute_idx(
            options["energy_schema"],
