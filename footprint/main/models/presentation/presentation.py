...
class Presentation(Name, ScopedKey, Deletable, RelatedCollectionAdoption):
    ...
    def donor(self):
        ...
        try:
            return (
                ...
                and parent_config_entity.presentation_set.filter(key=self.key)
                .select_subclasses()
                .get()
            )
    ...
    def donees(self):
        ...
        # each child
        return flat_map(
            lambda child_config_entity: child_config_entity.presentation_set.filter(
                key=self.key
            ).select_subclasses(),
    ...
    def computed_presentation_media(self, **query_kwargs):
        ...
        """
        return (
            self._computed(self.presentation_media_alias, **query_kwargs)
            .all()
            .select_subclasses()
        )
    ...
    def add_presentation_media(self, *presentation_media):
        ...
        logger.debug(
            ...
            % (
                ...
                self.id,
                ", ".join(
                    map_property(self.computed_presentation_media(), "db_entity_key")
                ),
                ...
                (
                    ", ".join(
                        map_property(
                            self.donor().computed_presentation_media(), "db_entity_key"
                        )
                    )
                    if self.donor()
                    else ""
    ...
    def db_entities(self):
        ...
        :return:
        """
        return self.config_entity.computed_db_entities().filter(
            key__in=map(
                lambda presentation_media: presentation_media.db_entity_key,
