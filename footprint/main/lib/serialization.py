...
def cond_deep_flat_map_iterable(iterable):
    if type(iterable) == dict:
        ...
        flattened_dict = remove_keys(
            merge(iterable, iterable.get("fields", {})), ["fields"]
        )
        ...
        return dict(
            [
                (
                    key,
                    (
                        cond_deep_flat_map_iterable(value)
                        if is_list_tuple_or_dict(value)
                        else value
                    ),
                )
        ...
        return map(
            lambda item: (
                cond_deep_flat_map_iterable(item)
                if is_list_tuple_or_dict(item)
                else item
            ),
            iterable,
        )
