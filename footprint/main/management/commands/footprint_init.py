...
from footprint.main.models.database.information_schema import (
    InformationSchema,
    PGNamespace,
)
...
from footprint.main.models.application_initialization import (
...
from footprint.main.models.presentation.layer_selection import (
...
from footprint.main.publishing.analysis_module_publishing import (
    update_or_create_analysis_modules,
    on_config_entity_post_save_analysis_modules,
    on_config_entity_pre_delete_analysis_modules,
)
...
from footprint.main.publishing.data_import_publishing import (
    ...
    DeleteImportProcessor,
)
from footprint.main.publishing.db_entity_publishing import (
    update_or_create_db_entity_and_interest,
    on_config_entity_post_save_db_entity,
    crud_db_entities,
)
from footprint.main.publishing.policy_publishing import (
    on_config_entity_post_save_policy,
)
from footprint.main.publishing.result_publishing import (
    on_config_entity_post_save_result,
)
from footprint.main.publishing.tilestache_publishing import (
    on_config_entity_post_save_tilestache,
)
from footprint.main.publishing.user_publishing import (
...
from footprint.main.publishing.layer_publishing import (
    on_config_entity_post_save_layer,
    delete_layer_selections,
    create_layer_selections,
)
...
def make_option_list():
    return (
        ...
        make_option(
            ...
            action="store_true",
            default=False,
            help="Skips layer publisher",
        ),
        make_option(
            "--noimport",
            action="store_true",
            default=False,
            help="Skips data import publisher",
        ),
        make_option(
            "--noinitializer",
            action="store_true",
            default=False,
            help="Skips initializer publishers",
        ),
        make_option(
            "--notilestache", action="store_true", help="Skips tilestache publisher"
        ),
        make_option(
            "--nobuilt_form", action="store_true", help="Skips built_form publisher"
        ),
        make_option("--noresult", action="store_true", help="Skips result publisher"),
        make_option(
            "--nodb_entity", action="store_true", help="Skips db_entity publisher"
        ),
        make_option(
            "--noanalysis", action="store_true", help="Skips analysis publisher"
        ),
        ...
        make_option(
            ...
            action="store_true",
            default=False,
            help="Explicitly run result publisher",
        ),
        make_option(
            "--reresult",
            action="store_true",
        ...
        make_option(
            ...
            action="store_true",
            default=False,
            help="Explicitly run policy publisher",
        ),
        make_option(
            "--import",
            action="store_true",
            default=False,
            help="Explicitly run data_import publisher",
        ),
        make_option(
            "--db_entity",
            action="store_true",
        ...
        make_option(
            ...
            action="store_true",
            default=False,
            help="Explicitly run built_form publisher",
        ),
        make_option(
            "--rebuilt_form",
            action="store_true",
        ...
        make_option(
            ...
            action="store_true",
            default=False,
            help="Explicitly run layer publisher",
        ),
        make_option(
            "--analysis",
            action="store_true",
            default=False,
            help="Explicitly run analysis publisher",
        ),
        make_option(
            "--run_analysis",
            action="store_true",
        ...
        make_option(
            "--selections",
            action="store_true",
            help="Reset config_enitty selections",
            default=False,
        ),
        make_option(
            "--test_clone_scenarios",
            action="store_true",
            help="Clone scenarios to simulate a UI clone",
            default=False,
        ),
        make_option(
            "--test_upload_layers",
            action="store_true",
            help="Upload layers to simulate a UI upload",
            default=False,
        ),
        make_option(
            "--test_layer_from_selection",
            action="store_true",
            help="Tests creating a layer/db_entity from from a selection",
            default=False,
        ),
        make_option(
            "--inspect",
        ...
        make_option(
            ...
            action="store_true",
            default=False,
            help="Delete config_entities marked deleted",
        ),
        make_option(
            "--delete_clones",
            action="store_true",
        ...
        make_option(
            "--relayer_selection",
            action="store_true",
            help="Recreates all layer selections",
        ),
        make_option(
            "--class",
            help="Comma separated classes in the form FutureScenario, Project, etc",
        ),
        make_option(
            "--dump_config_entity_permissions",
            action="store_true",
            help="Dump all permissions for all config_entities, or for those limited by class and/or config_entity_keys",
        ),
    )
...
class FootprintInit(object):
    ...
    def run_footprint_init(self, *args, **options):
        ...
        if not settings.CELERY_ALWAYS_EAGER:
            raise Exception(
                "This command must run with settings.CELERY_ALWAYS_EQUAL = True. "
                "Add --settings=footprint.settings_init to the command line."
            )

        db_entity_keys = (
            options.get("db_entity_keys").split(",")
            if options.get("db_entity_keys")
            else None
        )
        # Replace so we can use options as kwargs
        options["db_entity_keys"] = db_entity_keys
        config_entity_keys = (
            options.get("config_entity_keys").split(",")
            if options.get("config_entity_keys")
            else None
        )
        # Replace so we can use options as kwargs
        ...
        limit_to_classes = map(
            lambda cls: resolve_model("main.%s" % cls),
            (options.get("class").split(",") if options.get("class") else []),
        )
            ...
            for cls in filter_classes(limit_to_classes):
                ...
                for config_entity in all_config_entities:
                    if options.get("delete_clones"):
                        db_entities = map(
                            ...
                            and (
                                isinstance(db_entity.feature_class_configuration, dict)
                                or db_entity.feature_class_configuration.generated
                            ),
                            config_entity.computed_db_entities(),
                        )

                        layers_to_remove = Layer.objects.filter(
                            layer_libraries__config_entity__in=[config_entity],
                            db_entity_interest__db_entity__key__in=map(
                                lambda db_entity: db_entity.key, db_entities
                            ),
                        )
                        ...
                        for db_entity in db_entities:
                            ...
                            try:
                                ...
                                logger.warn(
                                    "No feature class for db_entity %s could be created. Exception: %s"
                                    % (db_entity.name, e.message)
                                )
                            DeleteImportProcessor().drop_data(config_entity, db_entity)
                            db_entity.delete()

                if issubclass(cls, Scenario):
                    cloned_config_entities = cls.objects.filter(
                        origin_instance__isnull=False
                    )

                    # ConfigEntities and their schemas
                    if options.get("delete_clones") or options.get(
                        "delete_scenario_clones"
                    ):
                ...
                if options.get("delete_clones") and False:
                    for built_form_set in BuiltFormSet.objects.all():
                        built_form_set.built_forms.remove(
                            *built_form_set.built_forms.filter(
                                origin_instance__isnull=False
                            )
                        )
                    # BuiltForms
                    ...
                    BuildingAttributeSet.objects.annotate(
                        num_buildings=Count("building"),
                        num_buildingtypes=Count("buildingtype"),
                        num_placetypes=Count("building"),
                    ).filter(
                        num_buildings=0, num_buildingtypes=0, num_placetypes=0
                    ).delete()
                    Medium.objects.annotate(
                        num_built_form_sets=Count("builtform")
                    ).filter(
                        num_built_form_sets=0, key__startswith="built_form"
                    ).delete()
                    BuiltFormExample.objects.annotate(
                        num_built_form_sets=Count("builtform")
                    ).filter(num_built_form_sets=0).delete()
        ...
        if options.get("save_db_entity_interests"):
            ...
            for config_entity in filter_config_entities(**options):
                for db_entity_interest in config_entity.owned_db_entity_interests(
                    **dict(db_entity__key__in=db_entity_keys) if db_entity_keys else {}
                ):
            ...
            if not options.get("skip"):
                if options.get("nodb_entity"):
                    disable_signal_handler(
                        resolvable_module_attr_path(
                            config_entity_publishing.__name__,
                            "post_save_config_entity_initial",
                        ),
                        on_config_entity_post_save_db_entity,
                ...
                if options.get("noimport"):
                    ...
                    disable_signal_handler(
                        resolvable_module_attr_path(
                            config_entity_publishing.__name__,
                            "post_save_config_entity_db_entities",
                        ),
                        on_config_entity_post_save_data_import,
                ...
                if options.get("notilestache"):
                    disable_signal_handler(
                        resolvable_module_attr_path(
                            config_entity_publishing.__name__,
                            "post_save_config_entity_layers",
                        ),
                        on_config_entity_post_save_tilestache,
                ...
                if options.get("noanalysis"):
                    disable_signal_handler(
                        resolvable_module_attr_path(
                            config_entity_publishing.__name__,
                            "post_save_config_entity_imports",
                        ),
                        on_config_entity_post_save_analysis_modules,
                ...
                if options.get("nouser"):
                    ...
                    disable_signal_handler(
                        resolvable_module_attr_path(
                            db_entity_publishing.__name__, "post_save_db_entity_initial"
                        ),
                        on_db_entity_post_save_user,
                ...
                (
                    ...
                    if not options.get("minimum_installation")
                    else minimum_initialization(limit_to_class=limit_to_classes)
                )
                ...
                if options.get("initializer"):
                    ...
                    if options.get("rebehavior"):
                        ...
                        for behavior in Behavior.objects.all():
                            template_feature_behavior = (
                                behavior.template_feature_behavior
                            )
                    ...
                    application_initialization(
                        limit_to_classes=limit_to_classes, no_post_save_publishing=True
                    )

                if (
                    options.get("user")
                    or options.get("db_entity_permissions")
                    or options.get("config_entity_permissions")
                ):
                    if options.get("config_entity_permissions") or not options.get(
                        "db_entity_permissions"
                    ):
                        for config_entity in filter_config_entities(**options):
                            on_config_entity_post_save_group(
                                None, instance=config_entity
                            )
                        for config_entity in filter_config_entities(**options):
                            on_config_entity_post_save_user(
                                None, instance=config_entity
                    ...
                    if not options.get("config_entity_permissions"):
                        for db_entity_interest in filter_db_entity_interests(**options):
                            on_db_entity_post_save_user(
                                None, instance=db_entity_interest
                            )
                ...
                if options.get("db_entity") or options.get("redb_entity"):
                    ...
                    missing_schemas = filter(
                        lambda x: x.key != "template_feature_behavior" and not x.schema,
                        DbEntity.objects.all(),
                    )
                    if len(missing_schemas) > 0:
                        raise Exception(
                            "No schemas for the following db_entities %s"
                            % map_property_path(missing_schemas, "key")
                        )
                ...
                if options.get("import"):
                    for config_entity in filter_config_entities(**options):
                        on_config_entity_post_save_data_import(
                            None, instance=config_entity, db_entity_keys=db_entity_keys
                        )
                ...
                if (
                    options.get("layer")
                    or options.get("relayer")
                    or options.get("relayer_selection")
                ):
                    for config_entity in filter_config_entities(**options):
                        if options.get("relayer"):
                            layers = Layer.objects.filter(
                                **merge(
                                    dict(presentation__config_entity=config_entity),
                                    (
                                        Intersection(
                                            db_entity_interest__db_entity__key__in=db_entity_keys
                                        )
                                        if db_entity_keys
                                        else {}
                                    ),
                            ...
                            logger.critical(
                                "Deleting layer selections for config_entity %s and layers %s"
                                % (
                                    config_entity.name,
                                    ", ".join(
                                        map(lambda layer: layer.db_entity_key, layers)
                                    ),
                                )
                            )
                            delete_layer_selections(layers)
                            logger.critical(
                                "Creating layer selections for config_entity %s and layers %s"
                                % (
                                    config_entity.name,
                                    ", ".join(
                                        map(lambda layer: layer.db_entity_key, layers)
                ...
                if options.get("tilestache"):
                    for config_entity in filter_config_entities(
                        **merge(
                            options,
                            (
                                dict(limit_to_classes=[Scenario])
                                if not options.get("limit_to_classes")
                                else dict()
                            ),
                        )
                    ):
        ...
        if options.get("reconfig_entity"):
            for config_entity in Region.objects.all():
                logger.info(
                    "Re-saving region {config_entity}".format(
                        config_entity=config_entity.name
                    )
                )
                config_entity.save()
            for config_entity in Project.objects.filter(
                **dict(key__in=config_entity_keys) if config_entity_keys else dict()
            ):
                logger.info(
                    "Re-saving project {config_entity}".format(
                        config_entity=config_entity.name
                    )
                )
                config_entity.save()
            for config_entity in BaseScenario.objects.filter(
                **dict(key__in=config_entity_keys) if config_entity_keys else dict()
            ):
                logger.info(
                    "Re-saving base scenario {config_entity}".format(
                        config_entity=config_entity.name
                    )
                )
                config_entity.save()
            for config_entity in FutureScenario.objects.filter(
                **dict(key__in=config_entity_keys) if config_entity_keys else dict()
            ):
                logger.info(
                    "Re-saving future scenario {config_entity}".format(
                        config_entity=config_entity.name
                    )
                )
                config_entity.save()
            ...
            for config_entity in filter_config_entities(**options):
                for db_entity_key in db_entity_keys:
                    ...
                    if db_entities.count() == 1:
                        ...
                        logger.info(
                            "ConfigEntity: %s, DbEntity key: %s, Feature class: %s"
                            % (
                                config_entity.name,
                                db_entity.name,
                        ...
                        logger.info(
                            "Feature Class configuration from introspection: %s"
                            % feature_class_configuration
                        )
        ...
        if options.get("dump_behaviors"):
            for config_entity in filter_config_entities(**options):
                for db_entity in config_entity.computed_db_entities(
                    **dict(key__in=db_entity_keys) if db_entity_keys else dict()
                ):
                    logger.info(
                        "ConfigEntity: %s, DbEntity key: %s, Behaviors: %s"
                        % (
                            config_entity.name,
                            db_entity.name,
                            db_entity.feature_behavior.behavior.dump_behaviors(),
                        )
                    )
...
def create_scenario_clone(test_layer=False):
    ...
    for new_scenario_configuration in import_scenario_configurations:
        ...
        matches = scenario.__class__.objects.filter(
            key=new_scenario_configuration["key"]
        )
        if matches:
            on_config_entity_pre_delete_data_import(None, instance=matches[0])
            matches.delete()
    ...
    return sorted(
        map(
            lambda config_entity: (
                resolve_scenario(config_entity)
...
def delete_upload_layer(config_db_entity, config_entity):
    ...
    Layer.objects.filter(
        db_entity_interest__db_entity__key=config_db_entity.key
    ).delete()
...
def filter_config_entities(**options):
    ...
    if not _CACHED_CONFIG_ENTITIES:
        ...
        assert (
            len(connection.queries) < total_entities * 13
        ), "Got %d queries for %d config_entities " % (
            len(connection.queries),
            len(_cached_config_entities),
        )
    ...
    result = sort_config_entities(
        filter(
            lambda config_entity: not options.get("limit_to_classes")
            or isinstance(config_entity, tuple(options.get("limit_to_classes"))),
...
def filter_db_entity_interests(**options):
    ...
    :return:
    """
    return DbEntityInterest.objects.filter(
        **compact_kwargs(
...
def disable_signal_handler(signal_ref_path, handler, uid, limit_to_classes):
    ...
    disable_signal_handler_for_celery.apply_async(
        args=(
            signal_ref_path,
            full_module_path(handler),
            uid,
            map(lambda cls: full_module_path(cls), limit_to_classes),
        ),
...
def test_upload_layer(config_db_entity, config_entity):
    ...
    db_entity_interest = update_or_create_db_entity_and_interest(
        config_entity, config_db_entity
    )[0]

    logger.info("Getting the layer")
    layer = Layer.objects.get(
        presentation__config_entity=config_entity, db_entity_interest=db_entity_interest
    )
...
@app.task
def disable_signal_handler_for_celery(
    signal_ref_path, handler_path, uid, limit_to_classes_paths
):
    for cls in filter_classes(limit_to_classes_paths):
        resolve_module_attr(signal_ref_path).disconnect(
            resolve_module_attr(handler_path), resolve_module_attr(cls), True, uid
        )
