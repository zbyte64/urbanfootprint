...
class GeoJsonProcessor(ImportProcessor):
    ...
    def importer(self, config_entity, db_entity, **kwargs):
        ...
        if self.seed_data:
            data = geojson.loads(
                jsonify(self.seed_data), object_hook=geojson.GeoJSON.to_instance
            )
        else:
        ...
        feature_class_configuration = feature_class_creator.feature_class_configuration_from_geojson_introspection(
            data
        )
        ...
        for feature in map(
            lambda feature: self.instantiate_sub_class(feature_class, feature),
            data.features,
        ):
            feature.save()
    ...
    def instantiate_sub_class(self, feature_class, feature):
        ...
        field_dict = map_to_dict(
            lambda field: [field.name, feature.properties[field.name]],
            filter(
                lambda field: feature.properties.get(field.name, None),
