...
from footprint.client.configuration.fixture import resolve_fixture
...
post_save_layer_initial = Signal()
...
def on_layer_post_save_db_entity_process_layer(sender, **kwargs):
    ...
    for layer_library_configuration in layer_configuration_fixture.layer_libraries():
        ...
        logger.info(
            "Handler: Layer Library Configuration: %s" % layer_library_configuration
        )
        # Update/Create the layer library and layers
        _update_or_create_layer_library_and_layers(
            config_entity, layer_library_configuration, db_entity_keys=[db_entity.key]
...
def post_save_layer_initial_publishers(cls):
    ...
    """
    post_save_layer_initial.connect(
        on_layer_post_save_process_layer, cls, True, "process_on_layer_post_save"
    )
    post_save_layer_initial.connect(
        tilestache_publishing.on_layer_post_save_tilestache,
        cls,
        True,
...
def _update_or_create_child_layers(config_entity, db_entity_key):
    ...
    if not isinstance(config_entity, Scenario):
        logger.debug(
            "_update_or_create_child_layers: db_entity_key %s, config_entity %s"
            % (db_entity_key, config_entity.name)
        )
        ...
        if len(some_layers) == 0:
            ...
            raise Exception(
                "Layer expected to exist for db_entity %s, but does not" % db_entity_key
            )
        ...
        def update_layer_of_scenario(scenario):
            logger.info(
                "Updating layer of db_entity_key %s, Scenario %s"
        ...
        layers = map(update_layer_of_scenario, config_entity.children())

        Layer._no_post_save_publishing = False
    else:
        layers = Layer.objects.filter(
            presentation__config_entity=config_entity, db_entity_key=db_entity_key
        )
        if len(layers) != 1:
...
def on_db_entity_post_save_layer(sender, **kwargs):
    ...
    scoped_config_entities = (
        [config_entity]
        if isinstance(config_entity, Scenario)
        else config_entity.children()
    )
...
def on_layer_post_save(sender, **kwargs):
    ...
    db_entity_interest = config_entity.computed_db_entity_interests(
        db_entity__key=layer.db_entity_key, with_deleted=True
    )[0]
    ...
    user = (
        layer.updater
        if layer.updater
        else get_user_model().objects.get(username=UserGroupKey.SUPERADMIN)
    )
...
def on_config_entity_post_save_layer(sender, **kwargs):
    ...
    logger.info(
        "Handler: on_config_entity_post_save_layer for %s" % config_entity.full_name
    )
    ...
    layer_library_configurations = FixtureList(
        client_layer_fixture.layer_libraries()
    ).matching_scope(class_scope=config_entity.__class__)
    logger.info(
        "Processing LayerLibrary Configurations %s"
        % ", ".join(
            map(
...
def _update_or_create_layer_library_and_layers(
    config_entity, layer_library_configuration, **kwargs
):
    """
    ...
    logger.info(
        "Update/Create LayerLibrary %s for %s"
        % (layer_library_configuration.key, config_entity.full_name)
    )
    layer_library, updated, created = LayerLibrary.objects.update_or_create(
        ...
        defaults=dict(
            ...
            description=layer_library_configuration.description.format(
                config_entity.name
            ),
        ),
    )
    logger.info(
        "Updated/Created LayerLibrary %s for %s"
        % (layer_library_configuration.key, config_entity.full_name)
    )
    ...
    if layer_library_configuration.key != LayerLibraryKey.DEFAULT:
        ...
        # libraries
        layers = filter(
            lambda layer: not layer.configuration.get("library_keys")
            or layer_library_configuration.key
            in layer.configuration.get("library_keys"),
            config_entity.presentation_set.filter(key=LayerLibraryKey.DEFAULT)
            .select_subclasses()
            .get()
            .layers_of_owned_db_entities(),
        )
        ...
        if not config_entity.origin_instance:
            ...
            owned_db_entities = filter(
                lambda db_entity: db_entity_key_to_layer_configuration.has_key(
                    db_entity.key
                ),
                config_entity.owned_db_entities(),
            )
            logger.info(
                "Computed DbEntityInterests: %s"
                % ", ".join(
                    map(
                        lambda db_entity_interest: ":".join(
                            [
                                db_entity_interest.db_entity.key,
                                db_entity_interest.db_entity.schema,
                            ]
                        ),
            ...
            logger.info(
                "Processing Configured DbEntities: %s"
                % ", ".join(map(lambda db_entity: db_entity.key, owned_db_entities))
                if owned_db_entities
                else "None"
            )

            layers = map(
                lambda db_entity: update_or_create_layer(
                    layer_library, db_entity_key_to_layer_configuration[db_entity.key]
                ),
                owned_db_entities,
            )
        else:
            ...
            logger.info(
                "Processing Layers of Configured DbEntities: %s"
                % ", ".join(map(lambda db_entity: db_entity.key, owned_db_entities))
                if owned_db_entities
                else "None"
            )
    ...
    key_to_layer_lookup = map_to_dict(
        lambda layer: [layer.db_entity_key, layer], layer_library.layers.all()
    )
    layers_to_add = filter(
        lambda layer: layer.db_entity_key not in key_to_layer_lookup, layers
    )
...
def update_or_create_layer(
    ...
    owning_db_entity_interest = DbEntityInterest.objects.get(
        config_entity=layer_library.config_entity, db_entity__key=db_entity_key
    )
    ...
    if not layer_configuration:
        layer_configuration = resolve_layer_configuration(
            layer_library.config_entity_subclassed, db_entity_key, existing_layer
        )
    ...
    try:
        db_entity = layer_library.config_entity_subclassed.computed_db_entities().get(
            key=db_entity_key
        )
    except Exception as e:
        raise Exception(
    ...
    owning_db_entity_interest = DbEntityInterest.objects.get(
        config_entity=config_entity, db_entity__key=db_entity_key
    )
    ...
    layer, created, updated = Layer.objects.update_or_create(
        ...
        defaults=dict(
            ...
            configuration=dict(
                ...
                sort_priority=(
                    layer_configuration.sort_priority
                    if layer_configuration.sort_priority > 0
                    else 100
                ),
            ...
            creator=dummy_user,
            updater=(
                layer_configuration.creator
                if hasattr(layer_configuration, "creator")
                else get_user_model().objects.filter()[0]
            ),
        ),
    )
    # this will update or create the layer style and assign it to the layer
    update_or_create_layer_style_from_configuration(
        layer_library.config_entity, db_entity, layer, layer_configuration
    )
...
def update_or_create_layer_selections(
    layer_libraries=None, config_entity=None, layers=None, users=None
):
    """
    ...
    """
    if not layers:
        layer_libraries_of_config_entity = (
            layer_libraries
            if layer_libraries
            else LayerLibrary.objects.filter(config_entity=config_entity)
        )
...
def resolve_layer_configuration(config_entity, db_entity_key, layer):
    ...
    if layer and layer.configuration:
        layer_configuration = LayerConfiguration(
            **merge(
                ...
                (
                    dict(
                        built_form_set_key=layer.configuration.get(
                            "built_form_key", None
                        ),
                        sort_priority=layer.configuration.get("sort_priority", None),
                        attribute_sort_priority=layer.configuration.get(
                            "attribute_sort_priority", None
                        ),
                        column_alias_lookup=layer.configuration.get(
                            "column_alias_lookup", None
                        ),
                    )
    ...
    # if the the layer configuration doesn't exist, utilize the default layer configuration
        ...
        layer_configuration = copy.copy(
            list(client_layer_configuration.import_layer_configurations(geometry_type))[
                0
            ]
        )
...
def on_post_save_built_form_layer(sender, **kwargs):
    ...
    for layer_library in layer_libraries:
        ...
        existing_layers = layer_library.layers.filter(
            db_entity_key__in=built_form_dependent_db_entity_keys
        )
        ...
        for existing_layer in existing_layers:
            update_or_create_layer(
                layer_library,
                db_entity_key=existing_layer.db_entity_key,
                skip_layer_selection=True,
            )
        ...
        try:
            ...
            if layer_selection_class:
                if hasattr(layer_selection_class.features, "through"):
                    layer_selection_features_class = (
                        layer_selection_class.features.through
                    )
...
def update_or_create_layer_style_from_configuration(
    config_entity, db_entity, layer, layer_configuration
):
    """
    ...
    logger.info(
        "Config Entity %s. layer_configuration %s. layer %s"
        % (config_entity, layer_configuration.__dict__, layer)
    )

    layer_fixture.update_or_create_layer_style(
        config_entity, layer_configuration, layer
    )
