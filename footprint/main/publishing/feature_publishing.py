...
from footprint.main.publishing.analysis_module_publishing import (
    on_feature_post_save_analysis_modules,
)
...
from footprint.main.publishing.tilestache_publishing import (
    on_feature_post_save_tilestache,
)
...
post_save_feature_initial = Signal()
...
def post_save_feature_initial_publishers(cls):
    ...
    """
    post_save_feature_initial.connect(
        on_feature_post_save_analysis_modules,
        cls,
        True,
        "analysis_module_on_feature_post_save",
    )
    post_save_feature_initial.connect(
        on_feature_post_save_tilestache, cls, True, "tilestache_on_feature_post_save"
    )
...
def on_feature_post_save(sender, **kwargs):
    ...
    logger.info(
        "Handler: post_save_feature for config_entity {config_entity}, db_entity_key {db_entity_key}, and user {username}.".format(
