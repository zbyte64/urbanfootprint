...
def on_config_entity_post_save_analysis_modules(sender, **kwargs):
    ...
    logger.info(
        "Handler: on_config_entity_post_save_analysis_module. ConfigEntity: %s"
        % config_entity.name
    )
...
def update_or_create_analysis_modules(config_entity, **kwargs):
    ...
    for (
        analysis_module_configuration
    ) in analysis_module_fixture.default_analysis_module_configurations():
...
def on_db_entity_post_save_analysis_modules(sender, **kwargs):
    ...
    config_entities = (
        [main_config_entity]
        if isinstance(main_config_entity, Scenario)
        else [main_config_entity] + list(main_config_entity.children())
    )
    ...
    for config_entity in config_entities:
        ...
        for analysis_module in analysis_modules:
            for (
                analysis_tool
            ) in analysis_module.analysis_tools.all().select_subclasses():
                ...
                if analysis_tool.behavior and behavior.has_behavior(
                    ...
                    logger.info(
                        "Updating AnalysisTool %s for AnalysisModule %s for ConfigEntity %s"
...
def on_analysis_module_post_save(sender, **kwargs):
    ...
    logger.info(
        "Post Save for AnalysisModule %s for ConfigEntity %s"
    ...
    if not kwargs.get("created", None):
        ...
        if (
            not analysis_module._started
            and not analysis_module._no_post_save_task_run
