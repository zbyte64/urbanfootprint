...
def resolve_layers(layer_libraries, **kwargs):
    ...
    """
    logger.info(
        "Searching for Layer in LayerLibraries: {0}".format(
            ", ".join([l.key for l in layer_libraries])
        )
    )
    ...
    logger.info(
        "Found Layers to Invalidate: %s" % ", ".join([l.name for l in filtered_layers])
    )
...
def on_config_entity_post_save_tilestache(sender, **kwargs):
    ...
    logger.info(
        "Handler: on_config_entity_post_tilestache. ConfigEntity: %s"
        % config_entity.name
    )
...
def on_feature_post_save_tilestache(sender, **kwargs):
    ...
    db_entity_interest = config_entity.computed_db_entity_interests(
        db_entity__key=features[0].db_entity_key
    )[0]
    db_entity = db_entity_interest.db_entity
    logger.info(
        "Handler: on_feature_post_save_tilestache for config_entity %s and db_entity %s"
    ...
    scenarios = (
        config_entity.descendants_by_type(Scenario)
        if not isinstance(config_entity, Scenario)
        else [config_entity]
    )
    if not scenarios:
    ...
    logger.info(
        "Handler: on_feature_post_save_tilestache. Invalidate layer of scenarios %s"
        ...
        if subclassed_layer_style:
            ...
            for style_attribute in subclassed_layer_style.style_attributes.all():
                ...
                if not ignore_raster_layer:
                    ...
                    tilestache_layers = [
                        create_raster_layer(layer, style_attribute_id, cartocss)
                    ] + tilestache_layers
...
def create_vector_layer(config_entity, layer, attribute):
    ...
    vector_layer = build_vector_layer_config(
        merge(
            connection,
            dict(
                query=query,
    ...
    return TSLayer(
        key="layer_{0}_{1}_vector".format(layer.id, attribute), value=vector_layer
    )
...
def create_raster_layer(layer, attribute_id, cartocss):
    ...
    layer_key = "layer:{layer},attr_id:{attribute},type:{type}".format(
        layer=layer.id, attribute=attribute_id, type="raster"
    )
...
def create_query(attribute, config_entity, layer):
    ...
    try:
        query = str(
            feature_class.objects.values(*unique(["wkb_geometry", attribute])).query
        )
    ...
    updated_query = (
        query.replace(
            ...
            '{column_alias}" as {attribute}'.format(
                column_alias=column_alias, attribute=attribute
            ),
            1,
        )
        if column_alias
...
def create_layer_selection(config_entity, layer, attribute_id):
    ...
    logger.info(
        "Get/Create layer_selection for config_entity %s, layer %s, users %s"
        % (
            config_entity.key,
            layer.db_entity_key,
            ",".join(map(lambda user: user.username, users)),
        )
    )
    ...
    id_field = map(
        lambda field: field.name + "_id",
        layer_selection.feature_class._meta.parents.values(),
    )[0]

    vector_selection_layer = build_vector_layer_config(
        parameters=merge(
            connection,
            dict(query=query, column="wkb_geometry", user_id_lookup=user_id_lookup),
        ),
...
def style_id(layer, style_attribute):
    ...
    return "style__{0}__{1}__{2}__{3}".format(
        layer_library.config_entity_subclassed.id,
        layer_library.id,
        layer.db_entity_key,
        style_attribute.id,
    )
...
def render_attribute_styles(layer, style_attribute):
    ...
    layer.rendered_medium[style_attribute.id]["cartocss"] = make_carto_css(
        layer, style_attribute
    )
...
def make_mml(layer, style_attribute):
    ...
    attribute = (
        ...
        if style_attribute.attribute is not None
        else "wkb_geometry"
    )
...
def cartocss_data(layer, style_attribute):
    ...
    """
    logger.debug(
        "Handler: Generating Cartocss for %s. Style Attribute: %s"
        % (layer.medium_subclassed, style_attribute)
    )
    ...
    return {"id": "{0}.mss".format(style_id(layer, style_attribute)), "data": cartocss}
...
def on_post_analytic_run_tilestache(sender, **kwargs):
    ...
    logger.info(
        "Handler: on_post_analytic_run_tilestache. ConfigEntity: %s. Module %s"
        % (config_entity.name, module)
    )
    # Find the Default LayerLibrary so that we can invalidate all Layers or those matching our specified
    # DbEntity keys
    layer_library = LayerLibrary.objects.get(
        config_entity=config_entity, key=LayerLibraryKey.DEFAULT
    )
    ...
    layers = layer_library.layers.filter(
        db_entity_interest__db_entity__key__in=dependent_db_entity_keys
    )
...
def on_post_save_built_form_tilestache(sender, **kwargs):
    ...
    logger.info(
        "Processing TileStache updates for built forms: {0}".format(
