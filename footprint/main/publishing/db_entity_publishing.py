...
from footprint.main.models.geospatial.db_entity_configuration import (
...
from footprint.main.publishing import (
    data_import_publishing,
    layer_publishing,
    result_publishing,
    analysis_module_publishing,
    user_publishing,
    tilestache_publishing,
)
...
post_save_db_entity_initial = Signal()
# All signals that can run after data imports run
post_save_db_entity_import = Signal()
...
signal_proportion_lookup = dict(
    ...
    post_save_db_entity_initial=0.50,
    # layers and dataImports run in parallel
    post_save_db_entity_import=0.50,
)
...
def post_save_db_entity_initial_publishers(cls):
    ...
    """
    post_save_db_entity_initial.connect(
        data_import_publishing.on_db_entity_post_save_data_import,
        cls,
        True,
        "data_import_on_db_entity_post_save",
    )
    post_save_db_entity_initial.connect(
        user_publishing.on_db_entity_post_save_user,
        cls,
        True,
...
def post_save_db_entity_import_publishers(cls):
    ...
    """
    post_save_db_entity_import.connect(
        result_publishing.on_db_entity_post_save_result,
        cls,
        True,
        "result_on_db_entity_post_save",
    )
    # this will handled in layer_publishing and tilestache publishing post save
    post_save_db_entity_import.connect(
        layer_publishing.on_layer_post_save_db_entity_process_layer,
        cls,
        True,
        "layer_on_db_entity_post_save",
    )
    post_save_db_entity_import.connect(
        tilestache_publishing.on_db_entity_post_save_tilestache,
        cls,
        True,
        "tilestache_on_db_entity_post_save",
    )
    post_save_db_entity_import.connect(
        analysis_module_publishing.on_db_entity_post_save_analysis_modules,
        cls,
        True,
        "analysis_module_on_db_entity_post_save",
    )
...
def on_config_entity_post_save_db_entity(sender, **kwargs):
    ...
    logger.info(
        "Handler: on_config_entity_post_save_db_entity. ConfigEntity: %s"
        % config_entity.name
    )
...
def on_db_entity_interest_post_save(sender, **kwargs):
    ...
    if kwargs.get("created", None):
        ...
        # in the ConfigEntity.
        db_entity_interest.config_entity.db_entities.filter(key=db_entity.key).exclude(
            id=db_entity.id
        ).delete()
        ...
        if not db_entity.schema or not db_entity.table:
            db_entity.schema = (
                db_entity.schema or db_entity_interest.config_entity.schema()
            )
    ...
    if not (
        ...
        feature_class_configuration = db_entity.feature_class_configuration = (
            db_entity.feature_class_configuration or FeatureClassConfiguration()
        )
    ...
    return post_save_publishing(
        ...
        update_setup_percent_complete=db_entity_interest.db_entity.setup_percent_complete
        == 0,
    ...
    if CrudKey.CLONE == crud:
        ...
        if config_entity.origin_instance:
            ...
            default_db_entities = filter(
                lambda db_entity: (
                    db_entity.key in db_entity_keys if db_entity_keys else True
                ),
...
def update_or_create_db_entities_and_interests(config_entity, *db_entities):
    ...
    db_entity_interests_and_created = map(
        lambda db_entity: update_or_create_db_entity_and_interest(
    ...
    created_db_entity_interests = map(
        lambda tup: tup[0],
        filter(
            lambda db_entity_interests_and_created: db_entity_interests_and_created[1],
            db_entity_interests_and_created,
        ),
    )
...
def update_or_create_db_entity_and_interest(config_entity, config_db_entity):
    ...
    if not db_entity.no_feature_class_configuration:
        feature_class_creator = FeatureClassCreator(
            config_entity, db_entity, no_ensure=True
        )
        db_entity.feature_class_configuration.geography_scope = (
            config_entity.id
...
def clone_or_update_db_entity_and_interest(
    config_entity,
    source_db_entity,
    db_entity,
    ...
    if existing_db_entity_interest:
        ...
        db_entity.__dict__.update(
            merge(
                model_dict(
                    existing_db_entity_interest.db_entity, include_primary_key=True
                ),
                dict(
                    _feature_behavior=existing_db_entity_interest.db_entity.feature_behavior
                ),
            )
        )
    if not db_entity._feature_behavior:
        ...
        feature_behavior = (
            ...
            if source_db_entity
            else FeatureBehavior(behavior=get_behavior("reference"))
        )
    ...
    db_entity.__dict__.update(
        merge(
            ...
            (
                remove_keys(model_dict(source_db_entity), ["key", "schema", "name"])
                if source_db_entity and override_on_update
                else {}
            ),
            # Set the clone source key if different than the target key
            (
                dict(source_db_entity_key=source_db_entity.key)
                if source_db_entity and source_db_entity.key != key
                else {}
            ),
            dict(
                ...
                feature_class_configuration=FeatureClassConfiguration(
                    **merge(
                        ...
                        remove_keys(
                            db_entity.feature_class_configuration.__dict__,
                            (
                                source_db_entity.feature_class_configuration.__dict__.keys()
                                if source_db_entity and override_on_update
                                else ["abstract_class_name"]
                            ),
                        ),
                        ...
                        dict(
                            feature_class_owner=(
                                source_db_entity.feature_class_configuration.feature_class_owner
                                if source_db_entity
                                else db_entity.feature_class_configuration.feature_class_owner
                            ),
                        ),
                    )
                )
