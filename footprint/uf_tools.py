...
def add_geom_idx(conn_string, schema, table, column="wkb_geometry"):
    query = "CREATE INDEX {0}_geom_idx on {1}.{0} using GIST ({2});".format(
        table, schema, column
    )
    executeSQL_now(conn_string, [query])
...
def getis_ord(schema, table, field, distance, nonzero_field="wkb_geometry"):
    ...
    stat_totals = """ select sum({2}), count(id_grid), avg({2}) from {0}.{1} where {2} <> 0;
        """.format(
        schema, table, field, distance, nonzero_field
    )
    ...
    cluster_tbl = """
                    ...
                    """.format(
        schema, table, field, distance, stat_total, N, mean_stat, nonzero_field
    )
    ...
    variance = """select variance(getis_ord_gi) from {0}.{1}_{2}_{3}m_tmp;""".format(
        schema, table, field, distance
    )
    ...
    """.format(
        schema, table, field, distance, variance
    )
